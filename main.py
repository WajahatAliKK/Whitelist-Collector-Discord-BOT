     ...
     if Modal_test.licenseKey:
        ...
        if df.shape[0] == 0:
            ...
            embed = discord.Embed(
                ...
                color=discord.Color.blurple())
            embed.add_field(name="✅ Wallet count has been successfully reset to `0` and all data has been erased.", value="Use the command `/start` to initialize the bot using a different configuration, if any", inline=False)
            await ctx.respond(embed=embed, ephemeral=True)
     else:
        ...
        for guild in bot.guilds:
                ...
                if channel.permissions_for(guild.me).send_messages:  # checking if you have permissions
                    ...
                    async def button3_callback(interaction):
                            ...
                            if df.shape[0] != 0:
                                for line in csv_reader:
                                    if line[2] != str(Modal_test.User_ID):
                                        ...
                                        flag = True
                                        embed = discord.Embed(
                                            title=f"You have removed your previous wallet address from the Whitelist.\n Please click ```Add``` to submit a new wallet address before the deadline.",
                                            color=discord.Color.red())
                        ...
                        if flag:
                            ...
                            embed = discord.Embed(
                                title="You have not submitted your wallet address yet. \nPlease click `Add` to submit your wallet address before the deadline.",
                                color=discord.Color.red())
                            await interaction.response.send_message(embeds=[embed], ephemeral=True)
            ...
            if channel.name == 'general':
                embed = discord.Embed(
                    ...
                    color=discord.Color.green())
                embed.add_field(
                    name=f"The following are the instructions to officially submit your wallet address that will be used for minting:",
                    value="\n\n:one: Click __**Add**__ to submit your wallet address\n\n:two: "
                          f"Click __**Check**__ if you would like to check the wallet address "
                          f"you have submitted\n\n:three: Click __**Delete**__ if you would like to remove your existing wallet address and submit an entirely new one",
                    inline=False)
...
async def on_ready():
    ...
    #             # print(message)
    #             button = Button(label='Add', style=discord.ButtonStyle.green, emoji='🙂')
    #             button2 = Button(label='Check', style=discord.ButtonStyle.gray)
    #             button3 = Button(label='Delete', style=discord.ButtonStyle.danger)
    #             view = View()
    #             view.add_item(button)
    #             view.add_item(button2)
    #             view.add_item(button3)
    ...
    #                         else:
    #                             flag = True
    #                             embed = discord.Embed(title="You left the Whitelist 🥺",
    #                                                   color=discord.Color.red())
    #                             await interaction.response.send_message(embeds=[embed], ephemeral=True)
    #                 if flag:
    #                     with open('Whitelist.csv', 'w', encoding='UTF8', newline='') as writer_file:
    ...
    #                         csv_writer.writerows(Updated)
    #                 else:
    #                     embed = discord.Embed(title="You have not registered Yet",
    #                                           color=discord.Color.red())
    #                     await interaction.response.send_message(embeds=[embed], ephemeral=True)
    #
    #             async def button2_callback(interaction):
    ...
    #                             flag = False
    #                     if flag:
    #                         embed = discord.Embed(title="Your Registered Whitelist Address :",
    #                                               color=discord.Color.green())
    #                         embed.add_field(name="Wallet Address", value=f"{Address}", inline=False)
    #                         await interaction.response.send_message(embeds=[embed], ephemeral=True)
    #                     else:
    #                         embed = discord.Embed(title="You Have Not Registered YET 😨😨",
    #                                               color=discord.Color.green())
    #                         embed.add_field(name="Hurry Up Register yourself", value=f"ASAP", inline=False)
    #                         await interaction.response.send_message(embeds=[embed], ephemeral=True)
    #
    #             async def button_callback(interaction):
    #                 modal = MyModal()
    ...
    #             button3.callback = button3_callback
    #             if channel.name == 'general':
    #                 embed = discord.Embed(title=f"Hi, i'm here to collect your wallet address",
    #                                       color=discord.Color.blurple())
    #                 embed.add_field(name="1.", value="Press Add if you haven't Whitelisted Yet", inline=False)
    #                 embed.add_field(name="2.", value="Press check if you want to see your WhiteList Address ",
    #                                 inline=False)
    #                 embed.add_field(name="3.",
    #                                 value="Press delete if you want to delete the Existing address from whitelist",
    #                                 inline=False)
        ...
        if Modal_test.currency == "Sol" or Modal_test.currency == "SOL" or Modal_test.currency == "sol":
            ...
            if Modal_test.balance < Modal_test.minimun_balance:
                ...
                embed.add_field(name="Your wallet balance is insufficient to qualify for the Whitelist.",
                                value=f'**Resubmit your wallet address once you have sufficient balance to qualify before the deadline.**',
                                inline=False)
                await interaction.response.send_message(embeds=[embed], ephemeral=True)
                return
            else:
                flag = file_submission()
                print(flag)
                if flag == 'Done':
                    embed = discord.Embed(
        ...
        if Modal_test.balance >= Modal_test.minimun_balance:
            if Modal_test.valid_wallet:
                ...
                if flag.isdigit():
                    embed = discord.Embed(title=f"ERROR ❌", color=discord.Color.red())
                    embed.add_field(
                        name=f"You have already submitted a wallet address. You can only submit **ONE (1)** wallet address.",
            ...
            embed = discord.Embed(title=f"ERROR ❌", color=discord.Color.red())
            embed.add_field(name="Your wallet balance is insufficient to qualify for the Whitelist.",
                            value='**Resubmit your wallet address once you have sufficient balance, to qualify before the deadline.**',
                            inline=False)
            await interaction.response.send_message(embeds=[embed], ephemeral=True)
...
def file_submission():
    ...
    with open('Whitelist.csv', 'a', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
...
# Hi {username}, i'm here to collect your wallet address
# 1. Click Add if you haven't add the button
# 2. Click check if you want to see your address and click
# 3. Click delete if you want to delete the existing address from whitelist
#                                     """, view=view)
